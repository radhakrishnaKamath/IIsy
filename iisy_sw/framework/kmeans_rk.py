...
X = [i[0:9] for i in Set]
# Y =[i[8] for i in Set]

# Test set Xt and Yt
Set2 = pd.read_csv(testfile)
Sett = Set2.values.tolist()
Xt = [i[0:9] for i in Set]
# Yt =[i[8] for i in Set]

class_names=['notAllowed','allowed']
feature_names=['proto','tcp_src','tcp_dst','udp_src','udp_dst','tcp_flags','ether_type','flags','size']
...
for i in range(len(centre)):
    ...
    model.write("(" + str(centre[i][0])+"," + str(centre[i][1])+"," + str(centre[i][2])+"," + str(centre[i][3])+"," + str(centre[i][4])+"," + str(centre[i][5])+"," + str(centre[i][6])+"," + str(centre[i][7])+"," + str(centre[i][8])+ ",)" )
    model.write(";\n")
